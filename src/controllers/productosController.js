...
async function crear(req, res) {
  const {
    nombre, descripcion, categoria_id, proveedor_id, marca_id,
    codigo_barras, imagen_url, imagenes,
    con_stock_inicial, stock, precio,
  } = req.body;
  ...
  try {
    ...
    // Si viene con stock inicial usa los valores, si no stock=0 precio=0
    const stockFinal  = con_stock_inicial ? (+stock  || 0) : 0
    const precioFinal = con_stock_inicial ? (+precio || 0) : 0
...
async function actualizar(req, res) {
  const { id } = req.params;
  const {
    nombre, descripcion, stock, categoria_id,
    proveedor_id, marca_id, codigo_barras, imagen_url, imagenes, estado
  } = req.body;
  try {
    const imgs = Array.isArray(imagenes) && imagenes.length > 0
      ? imagenes : (imagen_url ? [imagen_url] : []);
