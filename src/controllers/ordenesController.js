...
async function listar(req, res) {
  ...
  try {
    let query = `
      ...
      LEFT JOIN estados e ON o.estado_id = e.id WHERE 1=1
    `;
    const params = [];
...
async function cambiarEstado(req, res) {
  ...
  try {
    ...
    const orden = await client.query(
      'SELECT estado_id FROM ordenes_compra WHERE id=$1', [id]
    );
    if (!orden.rows.length)
      return res.status(404).json({ ok: false, mensaje: 'orden no encontrada' });
    // estado 11 = aprobado/recibido: actualizar stock automaticamente
    if (+estado_id === 11 && orden.rows[0].estado_id !== 11) {
    ...
    const r = await client.query(
      'UPDATE ordenes_compra SET estado_id=$1 WHERE id=$2 RETURNING *', [estado_id, id]
    );
    await client.query('COMMIT');
