...
async function crear(req, res) {
  const { nombre, apellido, email, password, telefono,
          rol_id, tipo_documento, numero_documento } = req.body;
  if (!nombre?.trim() || !apellido?.trim() || !email?.trim() || !password || !rol_id)
    return res.status(400).json({ ok: false, mensaje: 'todos los campos son obligatorios' });
  if (password.length < 6)
    return res.status(400).json({ ok: false, mensaje: 'contrasena minimo 6 caracteres' });
  try {
    const hash = await bcrypt.hash(password, 10);
    const r = await pool.query(
      `INSERT INTO usuarios
         (nombre,apellido,email,password,telefono,rol_id,tipo_documento,numero_documento)
...
async function actualizar(req, res) {
  const { id } = req.params;
  const { nombre, apellido, email, password, telefono,
          rol_id, tipo_documento, numero_documento, estado } = req.body;
  try {
    let query, params;
    if (password) {
      if (password.length < 6)
        return res.status(400).json({ ok: false, mensaje: 'minimo 6 caracteres' });
      const hash = await bcrypt.hash(password, 10);
      query = `UPDATE usuarios SET nombre=$1,apellido=$2,email=$3,password=$4,
               telefono=$5,rol_id=$6,tipo_documento=$7,numero_documento=$8,estado=$9
               WHERE id=$10 RETURNING id,nombre,apellido,email,rol_id,estado`;
      ...
      query = `UPDATE usuarios SET nombre=$1,apellido=$2,email=$3,telefono=$4,
               rol_id=$5,tipo_documento=$6,numero_documento=$7,estado=$8
               WHERE id=$9 RETURNING id,nombre,apellido,email,rol_id,estado`;
...
async function eliminar(req, res) {
  ...
  try {
    ...
    if (+pedidos.rows[0].total > 0)
      return res.status(400).json({ ok: false, mensaje: 'no se puede eliminar, tiene pedidos' });
