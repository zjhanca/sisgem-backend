...
async function crear(req, res) {
  ...
  try {
    // verificar que no sea rol cliente — los clientes se gestionan desde el módulo Clientes
    const rolCheck = await pool.query('SELECT nombre FROM roles WHERE id=$1', [rol_id]);
    if (rolCheck.rows[0]?.nombre?.toLowerCase().includes('cliente'))
      return res.status(400).json({ ok: false, mensaje: 'Los usuarios con rol cliente se crean desde el módulo Clientes' });
