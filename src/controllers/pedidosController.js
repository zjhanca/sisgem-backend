...
async function crear(req, res) {
  ...
  try {
    ...
    for (const item of productos) {
      const s = await client.query('SELECT stock FROM productos WHERE id=$1', [item.producto_id]);
    ...
    const pedidoRes = await client.query(
      ...
      [cliente_id||null, cliente_nombre||null, usuario_id, tipo_venta||'mostrador', total, notas||null]
    );
    ...
    for (const item of productos) {
      ...
      await client.query(
        `INSERT INTO pedido_productos
           (pedido_id,producto_id,cantidad,precio_unitario,subtotal,estado_id)
         VALUES ($1,$2,$3,$4,$5,1)`,
    ...
    if (domicilio?.direccion_id || domicilio?.direccion_manual) {
      await client.query(
        `INSERT INTO domicilios
           (pedido_id,direccion_id,direccion_manual,estado_id,tarifa_id,tarifa_aplicada)
         VALUES ($1,$2,$3,7,$4,$5)`,
        [pedido_id, domicilio.direccion_id||null, domicilio.direccion_manual||null,
         domicilio.tarifa_id||null, domicilio.tarifa_aplicada||0]
      );
    }
...
async function detalle(req, res) {
  ...
  try {
    ...
    const productos = await pool.query(`
      SELECT pp.*, pr.nombre AS producto, pr.codigo_barras
      FROM pedido_productos pp
      JOIN productos pr ON pp.producto_id = pr.id
      WHERE pp.pedido_id = $1
    `, [id]);
    res.json({ ok: true, datos: { ...pedido.rows[0], productos: productos.rows } });
  } catch (err) { res.status(500).json({ ok: false, mensaje: err.message }); }
}
